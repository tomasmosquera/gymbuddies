-- ============================================================================
-- Critical fix: run_weekly_evaluation() has been re-evaluating every group
-- EVERY DAY since 0070 switched its cron job from "Monday 5am UTC only" to
-- hourly ('0 * * * *'). 0073 then made it gate on "local hour = 7", but
-- neither 0070 nor 0073 ever added a day-of-week check — week_start/week_end
-- are a rolling "yesterday back 6 days" window recomputed fresh on every
-- call, so each new day produces a distinct week_start_date and sails right
-- past the (group_id, week_start_date) unique constraint that was supposed
-- to make this idempotent. Net effect confirmed in production: every group
-- has been getting a brand new weekly evaluation — including real penalty
-- charges — every single day instead of once a week.
--
-- Fix: gate on local ISO day-of-week = 1 (Monday) in addition to the hour,
-- and move the hour target to 8 per the admin's request (was 7).
-- ============================================================================

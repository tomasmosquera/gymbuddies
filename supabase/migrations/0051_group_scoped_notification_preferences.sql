-- ============================================================================
-- Notification preferences move from profiles (one global blob per user) to
-- group_members (one blob per user PER GROUP). A member can now mute e.g.
-- "GymBuds" while keeping "Gym Buddies" fully on. send_push_notification is
-- still the single choke point every notification goes through, so it's the
-- only place that needs to change how it checks the preference — but since
-- the preference is now group-scoped, EVERY call site must say which group
-- the notification belongs to (p_group_id, new 4th param, no default — every
-- one of the ~30 existing callers already has a group id in scope, so this
-- is a mechanical threading change, not new logic anywhere).
-- ============================================================================

alter table group_members add column notification_preferences jsonb not null default '{
  "group_activity": true,
  "money": true,
  "votes": true,
  "reminders": true,
...
-- Carry over each user's existing (global) preferences into every group
-- they're already in, so nobody's settings silently reset on this migration.
update group_members gm
  set notification_preferences = p.notification_preferences
  from profiles p
  where p.id = gm.user_id;

-- Self-service: lets a member update their own row's preferences without
-- going through the admin-only group_members_update_admin RLS policy.
create or replace function set_group_notification_preferences(p_group_id uuid, p_preferences jsonb)
...
-- The old 4-arg (pre-0042) and 5-arg (current) overloads both get replaced by
-- a single 6-arg version — drop explicitly since adding p_group_id changes
-- the parameter type list, which `create or replace` treats as a new
-- overload rather than a replacement.
drop function if exists public.send_push_notification(uuid[], text, text, jsonb, text);
...
begin
  ...
  into v_messages
  from push_tokens pt
  join group_members gm on gm.group_id = p_group_id and gm.user_id = pt.user_id
...
-- ============================================================================
-- The 30 functions below all call send_push_notification. Every call site
-- gets exactly one new argument, p_group_id => <the group id already in
-- scope at that point> — no other behavior changes.
-- ============================================================================

create or replace function admin_adjust_balance(p_group_id uuid, p_user_id uuid, p_amount numeric, p_note text default null)
...
begin
  ...
  perform send_push_notification(
    ...
    p_group_id => v_tx.group_id, p_category => 'money'
  );
end;
$$;
  ...
  if new.status = 'cancelled' and old.status = 'pending' then
    ...
    if v_recipient_ids is not null then
      perform send_push_notification(
        ...
        p_group_id => new.group_id, p_category => 'votes'
      );
    end if;
  end if;
  return new;
...
-- The only non-mechanical change in this migration: this cron job scans
-- ALL groups' scheduled leaves in one query, so it's restructured to loop
-- per group (each notification call must belong to exactly one group).
...
create or replace function set_attendance_override(p_group_id uuid, p_user_id uuid, p_date date, p_status text, p_note text default null)
...
-- ============================================================================
-- Nothing reads profiles.notification_preferences anymore — send_push_
-- notification checks group_members now, and the client (Permisos screen)
-- reads/writes the active group's membership row instead of the profile.
-- ============================================================================

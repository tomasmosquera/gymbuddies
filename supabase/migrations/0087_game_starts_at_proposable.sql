-- ============================================================================
-- game_starts_at was only ever settable once, at create_group time — an
-- admin who wants to push back "when we really start playing" for members
-- still in their protection period (see 0086) had no way to change it
-- afterward. Adds it to the same proposed_changes/p_changes jsonb pathway
-- every other rule field already uses (propose_rule_change /
-- apply_rule_change_direct), converted with the GROUP'S OWN CURRENT
-- timezone — same conversion create_group already does, just using
-- g.timezone instead of a fresh p_timezone argument, since here the group
-- already exists.
-- ============================================================================
...
begin
  ...
  update groups g
    set min_days_per_week = coalesce((v_proposal.proposed_changes ->> 'min_days_per_week')::int, g.min_days_per_week),
        ...
        game_starts_at = coalesce(
          ((v_proposal.proposed_changes ->> 'game_starts_at')::timestamp) at time zone g.timezone,
          g.game_starts_at
        )
...
begin
  ...
  update groups g
    set min_days_per_week = coalesce((p_changes ->> 'min_days_per_week')::int, g.min_days_per_week),
        ...
        game_starts_at = coalesce(
          ((p_changes ->> 'game_starts_at')::timestamp) at time zone g.timezone,
          g.game_starts_at
        )

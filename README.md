...
- Writes only through `submit_checkin(p_group_id, p_captured_at, p_latitude, p_longitude, p_location_accuracy_m, p_photo_path)` (self-service upsert, requires `is_voting_member`) — direct client table access is revoked. Retaking today's initial photo (the same `on conflict` upsert) normally leaves any already-recorded checkout untouched — **except** when that (group, date) currently has a `'failed'` `attendance_overrides` row (migration `0102`): that retake is treated as starting the day over, so it resets the checkout half (photo, timestamps, `workout_minutes`, `active_energy_kcal`) back to unset and deletes the `'failed'` override, so the fresh check-in counts again (and can be challenged again like any other, if anyone still doubts it). Scoped per group — a group where the day was never invalidated keeps whatever it already had.
...
- **Auto fan-out** (`profiles.auto_checkin_other_groups`, default on): submitting a check-in or checkout can also submit the same photo/location to every other group the member actively belongs to (`app/(app)/checkin/preview.tsx`'s `fanOutCheckinToOtherGroups`/`fanOutCheckoutToOtherGroups`, each group gets its own uploaded copy — storage RLS scopes read access by the `group_id` path segment). Gated on the member answering "yes" to a confirmation prompt shown every time the toggle is on and there's at least one other group to fan out to — **not** silent even with the toggle on, since a retake meant to fix one group's photo previously overwrote an already-independently-decided check-in in another group.
...
- **`cooperative`** (default): a full-group liquidation (`liquidate_group_now`, below) distributes the *gross* pool — nobody's own penalties ever shrink what the group as a whole has to split, they're redistributed equally among everyone else instead (migration `0098`). A single member *departing* mid-game (`leave_group`, the scheduled-notice sweep, or `admin_remove_member` with payout enabled) still uses the older mechanic: `pay_out_departing_member()` computes their `group_members.cooperative_weight`-proportional share of the *net* pool, pays it to them via a `'payout'` transaction, and spreads the difference across everyone remaining in proportion to *their* weights (cent-exact, remainder distributed one cent at a time) — a strict money-conservation invariant. `cooperative_weight` (numeric, default `1`) and `admin_set_cooperative_share_percent` still exist for this departure path, but are no longer exposed anywhere in the app (no admin UI edits it, so it stays at the default equal weight for everyone in practice).
...
#### Whole-group liquidation (migration `0065`, cooperative formula replaced by `0098`)
...
- **`liquidate_group_now(p_group_id, p_dry_run)`**: settles the *entire* group's pool today, in one shot — every active member's balance goes to `$0` and each one gets a `'payout'` transaction recording what they're actually owed (real money the admin still has to hand back outside the app, exactly like every other transaction in this system — the app never custodies funds). One function serves two purposes with the identical calculation, so a preview can never drift from reality:
  ...
  - **Pure `cooperative` mode**: each member gets their own remaining `balance` back, plus an equal share of every *other* eligible member's penalties charged since that member's last payout (never their own penalty money back) — `share_i = balance_i + (total_penalties − own_penalty_i) / (n − 1)`, with a single member (`n = 1`) simply getting their own penalty money back too. This always sums to exactly the *gross* pool (what was actually deposited), since a penalty never physically removes money from the group — it only lowers what the penalized member is owed, and this formula hands that difference to everyone else instead of leaving it stranded.
  - **`league`/`mixed`**: the non-League slice still splits by `cooperative_weight` exactly like a departure payout (see above) — this older mechanic was deliberately *not* extended to these two modes. The League slice is paid to the podium ranked by **today's** standing (not the cycle's original end date) using the same tie-merging logic as a normal cycle settlement, and any unclaimed prize percentage (splits under 100%, or fewer participants than prize slots) rolls into the cooperative-weight slice instead of being stranded. If the mode needs a running League cycle and there isn't one, it raises a clear error instead of silently returning zero for everyone.
  - The group itself is **not** deactivated or archived by a liquidation — it stays fully active with every member simply back at `$0`, free to keep depositing and playing.

Client-side: `app/(app)/profile/wallet.tsx` renders a **"Reparto de hoy"** card (via `useLiquidationPreview`, which just wraps the two `liquidate_group_now` calls) visible to every member, ranking rows by amount with a 🏆 next to podium places, and (admin only) a **"Liquidar ahora"** button that confirms with the exact per-member amounts before calling the real (non-dry-run) RPC. `useGroupAdminOverview`'s "Saldo total del grupo" figure mirrors the same gross-pool logic for `cooperative` groups (net balance + penalties charged since each member's last payout); other modes still show the plain net balance sum, matching what their liquidation actually pays out.
...
`useLeaderboard.ts` ranks members by **GB Score alone** (a Wilson-score-adjusted consistency %, never balance/money, never workout duration) — week / month / all-time periods, standard competition ranking (a genuine tie always shares the rank, e.g. `1, 1, 3`; see `rankMembersByConsistency` in `src/lib/domain/attendance.ts`). Each row also shows a purely informational `chargedAmount`: the frozen historical penalty sum for closed periods, or for the still-open current week, a **live "guaranteed-misses-only" projection** — a day only counts as a guaranteed miss once there literally aren't enough days left in the week to still hit quota, correctly clamped by `penalty_start_date` so a member in their grace period never shows a scary live number.

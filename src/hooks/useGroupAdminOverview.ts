...
export interface GroupAdminOverview {
  ...
  totalEnrollmentFeesCollected: number;
  pendingTransactionsCount: number;
  pendingExcusesCount: number;
  hasPendingRuleProposal: boolean;
  /** League mode only: members who left/were removed still holding a nonzero
   * balance the admin hasn't settled yet (see admin_settle_league_departure). */
  pendingLeagueDeparturesCount: number;
...
const EMPTY_OVERVIEW: GroupAdminOverview = {
  ...
  totalEnrollmentFeesCollected: 0,
  pendingTransactionsCount: 0,
  pendingExcusesCount: 0,
  hasPendingRuleProposal: false,
  pendingLeagueDeparturesCount: 0,
 ...
 * caller that needs this for several groups at once (e.g. useMyAdminOverview,
 * one group per admin membership) can call it in a loop/Promise.all without
 ...
 */
export async function fetchGroupAdminOverview(
  groupId: string,
  minDaysPerWeek: number,
  timezone: string
): Promise<GroupAdminOverview> {
  ...
  const payoutMode = groupRes.data?.payout_mode;
  const pendingLeagueDeparturesCount =
    payoutMode === 'league'
      ? members.filter((m) => (m.status === 'left' || m.status === 'removed') && m.balance !== 0).length
      : 0;

  const penalties = penaltiesRes.data ?? [];
  const totalPenaltiesCharged = penalties.reduce((sum, t) => sum + Math.abs(t.amount), 0);

  const totalEnrollmentFeesCollected = (enrollmentFeesRes.data ?? []).reduce(
    (sum, t) => sum + (t.enrollment_fee_amount ?? 0),
    0
  );
...
export function useGroupAdminOverview(groupId: string | null, minDaysPerWeek: number, timezone: string) {
  ...
  const refresh = useCallback(async () => {
    ...
    setIsLoading(true);
    const next = await fetchGroupAdminOverview(groupId, minDaysPerWeek, timezone);
    setOverview(next);
    setIsLoading(false);
